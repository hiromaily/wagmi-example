...
export const fetchBalances = async (args: balanceArgs): Promise<Record<string, string>> => {
  ...
  return Object.fromEntries(
    balances
      .filter(({ value }) => value?.result && value.result > 0)
      .map((balance) => [
        `${balance.chainId}:${getTokenFromAddr(balance.tokenAddr)}`,
